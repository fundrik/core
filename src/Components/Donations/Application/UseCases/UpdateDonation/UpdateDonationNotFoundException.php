...
/**
 * Thrown when update-donation targets a donation that no longer exists.
 *
 * @since 0.1.0
 */
final class UpdateDonationNotFoundException extends UpdateDonationException {
	...
	public function __construct( string $donation_id, ?Throwable $previous = null ) {
		...
		parent::__construct(
			...
			message: sprintf(
				'Cannot update donation "%s": donation does not exist.',
				$donation_id,
			),
			previous: $previous,
		);
