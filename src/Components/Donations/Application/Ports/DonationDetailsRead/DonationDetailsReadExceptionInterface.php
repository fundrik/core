<?php

declare(strict_types=1);

namespace Fundrik\Core\Components\Donations\Application\Ports\DonationDetailsRead;

use Fundrik\Core\Components\Shared\Application\Exceptions\FundrikApplicationExceptionInterface;

/**
 * Marks all exceptions that occur in donation details read operations.
 *
 * @since 0.1.0
 */
interface DonationDetailsReadExceptionInterface extends FundrikApplicationExceptionInterface {}
