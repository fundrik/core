...
/**
 * Thrown when activate-campaign targets a campaign that no longer exists.
 *
 * @since 0.1.0
 */
final class ActivateCampaignNotFoundException extends ActivateCampaignException {
	...
	public function __construct( string $campaign_id, ?Throwable $previous = null ) {
		...
		parent::__construct(
			...
			message: sprintf(
				'Cannot activate campaign "%s": campaign does not exist.',
				$campaign_id,
			),
			previous: $previous,
		);
